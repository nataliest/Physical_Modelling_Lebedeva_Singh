...
S4 = 1.56          # S is the stiffness of the string (stiffness of 90 makes bell sound)
# influence of s4 depends on the number of modes
c2 = 1e5           # c is speed of propagation
d1 = 0.13         # frequency independent damping 
d3 = -0.02         # frequency dependent damping

Mmax = 33         # Maximum number of modes
...
fs = 16000        # sampling frequency (1/s)
T = 1/float(fs)    # sampling period (s)

BLOCKSIZE = 32      # Number of frames per block
BLOCKSIZE = 128      # Number of frames per block
...
stop = False
l2 = 0.55 ########### THIS LENGTH IS FIXED

class Key:
    # Declares variables

	def __init__(self, leng1):
		self.b1 = np.zeros(Mmax)
		self.c1 = np.zeros(Mmax)
		self.c0 = np.zeros(Mmax)   
		self.gamma1 = np.zeros(Mmax)
		self.gamma2 = np.zeros(Mmax)
		...
		for m in range(1, Mmax+1):
			...
			self.gamma1[m-1] = m * pi / self.l1
			self.gamma2[m-1] = m * pi / l2

			self.X1[m-1] = ((self.gamma1[m-1] ** 2) + (self.gamma2[m-1] ** 2))

			self.sigma[m-1] = ( (d3 / 2) * self.X1[m-1] ) - (d1 / 2)

			self.omega[m-1] = sqrt(S4 * (self.X1[m-1] ** 2) + (c2 + ((d1 * d3) / 2) * (self.X1[m-1]) - (d1 ** 2 / 4)))
